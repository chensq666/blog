...
:::tip
- 由于使用单一状态树，应用的所有状态都集中到一个比较大的对象
- 为解决以上问题，Vuex可以将store分成 **模块（module）** 每个模块拥自己的state，getters，mutations，actions，甚至是嵌套子模块
:::
...
const moduleB = {
  ...
  actions: {
    fetchData({ state, getters, commit, dispath, rootState, rootGetters }){
...
:::tip
在store创建之后，可以使用`store.registerModule`方法来注册模块
:::
...
//注册嵌套模块
store.registerModule(['parent', 'child'], {
  //...
})

//  -> store.state.parent.child

```

## 热重载
:::tip
为提高开发效果，可以使用webpack的hmr api在开发过程中实现`getter，mutation，actions，mudule`的热重载，
:::

**对于mutation和module，需要使用`store.hotUpdate()`方法;**
...
if(module.hot){
  //使actions 和 mutation 成为可以热更新的模块
  module.hot.accept(['./mutations', './modules/a'], () => {
    // 获取到更新后的模块
    //babel6编译问题加上 .default
